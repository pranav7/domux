...
**Date:** 2026-09-10
**Status:** Accepted. Amends interface spec 6.2, 6.3, 6.4 and 6.8. **Amended by decision record 0028**, which combines the two boxes into the Navigator and removes the exited record: the split between the two surfaces, the pruning rule and the three near misses below all go with it. What survives is the red dot meaning waiting, which 0028 narrows further, and the top bar drawing no count.
