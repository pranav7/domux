...
/// MUX-27: every box closes, so the rectangle the reader sees is the rectangle the content
/// is measured into.
///
/// The inner rectangle is asserted beside the drawing, because that is the number the grid is
/// copied into and the number the pane's program is sized to: a box that drew a row it did not
/// report would leave a blank line, and one that reported a row it did not draw would write
/// over the rule.
#[test]
fn the_measurement_a_box_reports_is_the_one_it_drew() {
    let mut buf = Buffer::empty(Rect::new(0, 0, 20, 3));
    let inner = Boxed {
        title: "zsh",
        flag: None,
    ...
    assert_eq!(inner, Rect::new(1, 1, 18, 1));
    assert_eq!(
        Boxed::inner_of(Rect::new(0, 0, 20, 3)),
        inner,
