...
fn overlay_key(core: &mut Core, client: &ClientId, key: KeyEvent) {
    ...
    match overlay {
        ...
        Overlay::NameWorkspace(id) => {
            ...
            match key.key {
                // Not `close_overlay`, which the two arms above call: that clears the top
                // overlay and leaves `overlay_under` where it stands, and `n` on a row opens
                // this box over the switcher, so it would strand the switcher with nothing
                // drawing it and nothing closing it. `close_top_overlay` pops one level and
                // puts the keys where the frame marks them, which is what Esc in the switcher
                // already does.
        ...
        Overlay::Agents | Overlay::Confirm(ConfirmKind::DeleteWorkspace(_)) | Overlay::Usage => {
            // M1 never opens these. Task 18 adds the workspace confirmation, M3 the agents
            // overlay and M4 the usage one.
        }
    }
}
