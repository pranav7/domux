...
/// it, `pane_hit` finds the cell under the pointer with it, `Core::sync_pane_sizes` resizes a
/// running PTY to it and `Core::provisional_size` spawns one at it, so a pane is the same size
/// however it got there. A second answer is how the rows a program is given stop being the
/// rows drawn for it.
pub fn pane_screen(rect: domux_core::model::Rect) -> Size {
    Size {
