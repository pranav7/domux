...
impl Ctx<'_> {
    ...
    /// (decision record 0021). Everything else asks `view` and refuses.
...
pub fn dispatch(method: Method, ctx: &mut Ctx) -> Result<Value, ApiError> {
    ...
    match method {
        ...
        PaneClaimPassthrough(p) => pane::claim_passthrough(ctx, p),
        PaneReleasePassthrough(p) => pane::release_passthrough(ctx, p),
