...
Rust workspace. Crates live under `crates/`. `domux-term` holds the emulator trait and its libghostty-vt implementation; `m0-spike` is the M0 pane spike and will be lifted or deleted in M1.

## Build and test

- `cargo build --workspace` fetches two pinned inputs on first build and caches both under `~/.cache/domux`: the Ghostty source, cloned at the commit in `vendor/ghostty-pin.toml`, and the Zig that builds it, from `vendor/zig-pin.toml`. Set `DOMUX_GHOSTTY_SOURCE_DIR=/path/to/ghostty` to build a local checkout, and `DOMUX_ZIG=/path/to/zig` to use a local Zig of the same version. No Ghostty source lives in this repository; see docs/decisions/0002-ghostty-source-acquisition.md.
...
- `cargo test --workspace` runs unit, golden, and behavior tests.
...
- `cargo clippy --workspace --all-targets -- -D warnings` and `cargo fmt --all --check` must pass before a commit.
- The pane emulator is libghostty-vt and there is no second implementation; see docs/decisions/0001-terminal-emulator.md before touching crates/domux-term.
