...
- **A dot is drawn only while an agent is waiting on you, it is red, and it sits one cell after the name** in the slot the working word would take, which a word takes two cells out of. Working and compacting say themselves with the glyph and the word; idle and unknown draw nothing. The top bar draws no agent count.
