...
- **A dot is drawn only while an agent is waiting on you, it is red, and it sits two cells after the name** in the slot the working word would take. Working and compacting say themselves with the glyph and the word; idle and unknown draw nothing. The top bar draws no agent count. What turns `waiting` on is a Claude notification that stops for you: `0036` lists the five types, and any other type changes nothing.
