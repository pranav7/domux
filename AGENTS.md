...
- So a record exists whether or not the hooks are installed, and the end is seen either way, but **a lost hook is not made good later**. A `Stop` that never arrives leaves the row `working` with a turning glyph until the row goes quiet, ten minutes later, and the state it goes to is `unknown` rather than the `idle` the lost hook would have set (`0058`). A row on `working` for minutes, or on `unknown` in the middle of a turn, is what hook loss looks like; go and look rather than waiting.
- **A turn that died is a turn that ended.** Claude Code sends `StopFailure` in place of `Stop` when a turn dies on an API error, and `hooks::parse_claude` reads both as `Stop`, so the row goes idle and the error is not kept as a reason (`0057`). A turn cancelled before its request goes out sends no hook at all, and there is no such event to install.
- **A row nothing reports on goes quiet.** `working` and `compacting` are the two states only a hook can leave, so a record whose transcript has not been written for `quiet::AFTER`, ten minutes, moves to `unknown` on the tick (`0058`, `AgentEvent::Quiet`, `agents::quiet`). The transcript's modification time is the evidence and the last hook is not: a turn that calls no tool sends no hook while it writes the whole time. Ten minutes is measured against the longest real in-turn silence in the author's own transcripts, 191 seconds. `waiting` stands however long it takes, a record with no transcript never goes quiet, and the next hook takes the row back.
...
- **A running agent keeps its pane.** That walk needs the worker's tree to still pass the agent that started it, and a worker the shell that ran it left behind, or a session on a daemon of its own, has no such tree. So the same walk is read a second way: a hook that would take the pane from a record whose own process is alive, and that did not run under that process, changes no record (`0055`, `nested::runs_under`). A session id that changed under a process that did not, which is what `/clear` does, still takes the pane, because its hook ran under the record's process.
- **The agent in front of a pane can be behind a wrapper.** A version manager's shim, an npm launcher or a script of your own stays in front while the agent runs in its group, so the observer asks the process in front first and the rest of its group after, nearest first, and the record holds the agent's own process (`0056`, `agents::front`). A process that left the group, which is what a session on a daemon of its own has done, is not in the pane.
...
- `unseen` turns on at every arrival at waiting, and when an agent goes from working to idle, which is what `agent::attention` answers. It clears when the agent's pane is focused, when input reaches that pane, and when a tab is selected onto that pane, and on nothing else. Reading the records does not clear it. It reddens the dot, brightens a recap in the switcher, and lifts a row in `sorted_agents` (`0052`).
- **A dot is drawn only while an agent is waiting on you, it is `◉`, and it sits one cell after the name**, in the column the glyph takes (`0042`). **Its colour is whether you have looked**: red (`waiting_dot`) until you open the agent's pane, grey (`waiting_dot_seen`) after, under both built-in themes and any theme that keeps those two roles red and grey (`0052`). The dot itself stands until the agent is answered, so a row never falls silent while its agent is blocked. Working says itself with the turning star and the word, and compacting with a still `↓` whose colour breathes and its own word (`0050`), or either with the glyph alone in the sidebar's two forms (`0038`); idle and unknown draw nothing. The top bar draws no agent count. What turns `waiting` on is a Claude notification that stops for you: `0037` lists the five types, and any other type changes nothing.
- The working word wears a **band**, a bright wave that runs along it and back, carried over from V1 (`render::shimmer` and `theme::Shimmer`). One counter drives it, the glyph and the compacting arrow's breath: the band moves every tick, 70 ms since `0038`, the glyph turns every second one, and the arrow breathes once every 28 (`render::shimmer::breath` and `theme::Breath`). Read `docs/decisions/0032` before changing either rate, `0050` before changing the breath, and `0019` for why the ticker never stops.
...
- **A recap is an entry the agent wrote as a recap**, an `away_summary` or a `/recap`, and never the last thing the agent happened to say. The last one stands until the agent writes another. Claude's session name is the transcript's last `custom-title`. `0034` records all of it and replaces the recap half of `0027`.
- **A Codex session name is the newest line for its session id in `session_index.jsonl`**, which Codex's `/rename` appends to in the Codex home. The home is found from the rollout path the hooks send as `transcript_path`, never from the environment, and a blank name is skipped. Each manifest's `NameSource` says where its kind's name comes from. `0049` records why the index and not the rollout or Codex's database, and why domux has no rename of its own.
- **Nothing but the tick reads a transcript.** The recap lands minutes after the hook that ends the turn, so `agent.report` reads no file and `recap::poll` asks once a second. `session_index::poll` asks for Codex's names on the same tick. A read takes only the bytes the agent appended (`agents::tail`), so the cost does not follow the size of the file.
