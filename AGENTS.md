...
- **A dot is drawn only while an agent is waiting on you, it is red under both built-in themes and any theme that keeps `waiting_dot` red, it is `◉`, and it sits one cell after the name**, in the column the glyph takes (`0042`). Working says itself with the turning star and the word, and compacting with a still `↓` whose colour breathes and its own word (`0048`), or either with the glyph alone in the sidebar's two forms (`0038`); idle and unknown draw nothing. The top bar draws no agent count. What turns `waiting` on is a Claude notification that stops for you: `0037` lists the five types, and any other type changes nothing.
- The working word wears a **band**, a bright wave that runs along it and back, carried over from V1 (`render::shimmer` and `theme::Shimmer`). One counter drives it, the glyph and the compacting arrow's breath: the band moves every tick, 70 ms since `0038`, the glyph turns every second one, and the arrow breathes once every 28 (`render::shimmer::breath` and `theme::Breath`). Read `docs/decisions/0032` before changing either rate, `0048` before changing the breath, and `0019` for why the ticker never stops.
