...
- **A turn that died is a turn that ended.** Claude Code sends `StopFailure` in place of `Stop` when a turn dies on an API error, and `hooks::parse_claude` reads both as `Stop`, so the row goes idle and the error is not kept as a reason (`0057`). A turn cancelled before its request goes out sends no hook at all, and there is no such event to install, so that row does stay `working`.
